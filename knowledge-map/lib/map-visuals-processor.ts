...
export function processMapElementsForDisplay(
  currentNodes: Node[],
  currentEdges: Edge[],
  ...
  currentNodes.forEach(node => {
    ...
    } else { // Covers NotStarted and undefined/null levels
  ...
  if (allNodesEffectivelyNotStarted && currentNodes.length > 0) {
    // Find the node whose y-coordinate is closest to 0.
    // If there's a tie, prefer the node with the smaller x-coordinate.
    firstFocusNodeId = currentNodes.reduce((accNode, currentNode) => {
        const absCurrentY = Math.abs(currentNode.position.y);
        const absAccY = Math.abs(accNode.position.y);

        if (absCurrentY < absAccY) {
            return currentNode; // currentNode's y is closer to 0
        }
        if (absCurrentY === absAccY) {
            // y-distance to 0 is the same, tie-break with x (smaller x is preferred)
            if (currentNode.position.x < accNode.position.x) {
                return currentNode;
            }
        }
        return accNode; // accNode remains the best candidate
    }, currentNodes[0])?.id || null; // currentNodes[0] is the initial accumulator
  }


  const notStartedConnectedToFocusIds = new Set<string>();

  if (!allNodesEffectivelyNotStarted && understoodNodeIds.size > 0) { // Original logic: focus on understood nodes
  ...
  } else if (allNodesEffectivelyNotStarted && firstFocusNodeId) { // New logic: all NotStarted, focus on a primary node
    currentEdges.forEach(edge => {
  ...
  const processedNodes: Node[] = currentNodes.map(node => {
    ...
    } else { // Fallback for nodes that might not fit other categories (e.g., undefined level but not in notStartedNodeIds)
      opacity = FULLY_OPAQUE; 
    }
    return {
      ...node,
  ...
  const processedEdges: Edge[] = currentEdges.map(edge => {
    ...
    if (sourceIsUnderstood && targetIsUnderstood) {
      ...
      if (allNodesEffectivelyNotStarted && (sourceIsConnectedToFocus || targetIsConnectedToFocus) && !(sourceIsFirstFocus || targetIsFirstFocus)) {
      ...
      else { // General case for two not-started nodes not part of the initial focus logic
        opacity = EDGE_ISOLATED_NOT_STARTED_OPACITY;
      }
    } else { // Fallback for edges that don't fit other categories
      opacity = EDGE_FULLY_OPAQUE;
    }
    return {
      ...edge,
